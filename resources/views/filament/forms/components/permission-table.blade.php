...
<div x-data="{
    ...
    <!-- Header Controls -->
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-10 p-6 bg-gradient-to-br from-indigo-50/50 to-white rounded-3xl border border-indigo-100/50 shadow-sm relative overflow-hidden">
        <div class="relative z-10 flex items-start gap-4">
            ...
            <div>
                <h3 class="text-xl font-black text-slate-900 tracking-tight">Security Matrix</h3>
                <p class="text-sm text-slate-500 font-medium mt-1">Configure workspace-wide access policies with precision.</p>
            </div>
        </div>
        
        <div class="relative z-10 flex items-center gap-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
            <button type="button" @click="checkAll" class="flex items-center gap-2 px-5 py-2.5 text-xs font-black uppercase tracking-widest text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 hover:shadow-lg hover:shadow-indigo-200 transition-all active:scale-95 group">
                <x-heroicon-o-check-circle class="w-4 h-4" />
                Grant Everything
            </button>
            <button type="button" @click="uncheckAll" class="flex items-center gap-2 px-5 py-2.5 text-xs font-black uppercase tracking-widest text-slate-600 bg-slate-50 rounded-xl hover:bg-slate-100 border border-slate-200/50 transition-all active:scale-95 group">
        ...
        <!-- Decoration -->
        <div class="absolute -right-8 -bottom-8 w-32 h-32 bg-indigo-500/5 rounded-full blur-3xl"></div>
    </div>

    <!-- Matrix Table -->
    <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] overflow-hidden">
                    ...
                    @foreach(['c' => 'Create', 'r' => 'Read', 'u' => 'Update', 'd' => 'Delete'] as $char => $label)
                        <th class="py-6 px-2 text-center">
                            <div class="inline-flex flex-col items-center gap-1">
                                <span class="text-sm font-black text-slate-900">{{ strtoupper($char) }}</span>
                    ...
                    <tr class="hover:bg-indigo-50/20 transition-colors group">
                        <td class="py-5 px-8">
                            <div class="flex items-center gap-4">
                                <div class="flex-shrink-0 w-12 h-12 flex items-center justify-center bg-slate-50 rounded-2xl group-hover:bg-white group-hover:shadow-md group-hover:shadow-indigo-50 transition-all duration-300 ring-1 ring-slate-100 group-hover:ring-indigo-100">
                                    @svg($module['icon'], 'w-6 h-6 text-slate-400 group-hover:text-indigo-600 transition-colors')
                                </div>
                                <div class="truncate">
                                    <span class="block text-sm font-black text-slate-800 group-hover:text-indigo-950 transition-colors tracking-tight">{{ $module['label'] }}</span>
