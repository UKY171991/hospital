...
<div class="row">
    <div class="col-md-4">
        <div class="card card-primary">
            <div class="card-header"><b>Create Complaint Type</b></div>
            <div class="card-body">
                <form id="complaintTypeForm">
                    <input type="hidden" id="complaint_type_id" name="complaint_type_id">
                    <div class="form-group">
                        <label for="name">Complaint Name<span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="name" name="name" placeholder="ComplaintName" required>
                    </div>
                    <div class="form-group">
...
<script>
$(function() {
    let table = $('#complaintTypeTable').DataTable({
        processing: true,
        serverSide: false,
        ajax: {
    ...
    // Save or update
    $('#complaintTypeForm').submit(function(e) {
        e.preventDefault();
    ...
    // Edit
    $('#complaintTypeTable').on('click', '.editBtn', function() {
        let id = $(this).data('id');
        let name = $(this).data('name');
        let description = $(this).data('description');
        $('#complaint_type_id').val(id);
        $('#name').val(name);
        $('#description').val(description);
    });

    // Toggle status
    $('#complaintTypeTable').on('click', '.toggle-status', function() {
        let id = $(this).data('id');
        $.post(`{{ url('complaint-type/toggle-status') }}/${id}`, {_token: '{{ csrf_token() }}'}, function(res) {
